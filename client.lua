...
local function IsOverLimitCheck() return great >= limit and true or false end

local function DrawTextOnScreen(text, x, y)
	if IsPOverLimitCheck() == false then
...
Citizen.CreateThread(function() -- Update Ping on Screen
	while true do	if display then	DrawTextOnScreen(curPing .. 'ms' .. (IsPingOverLimit() and (' (Max: %dms)'):format(limit) or ''), 0.0009, 0.9798) end Citizen.Wait(5) end end)
